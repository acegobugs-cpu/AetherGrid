...
AETHER-GRID follows a **desired-state and reconciliation model**. The control plane (see `../controlPanel`) is the operator-facing side; this repository is **Phase 2**: the Edge Node Agent that represents a single Linux node in the fleet.
...
2. **State loop** — collects local machine state (hostname, OS, CPU, memory, uptime) and reports the agent's status and IP to the control plane, then retrieves the authoritative desired state.
3. **Command loop** — polls for pending commands, executes them with a bounded timeout, and reports the outcome.

The agent is resilient: a temporary control-plane outage is retried with backoff, an unknown identity triggers automatic re-registration, and a `RESTART_AGENT` command shuts the process down cleanly (for a supervisor to restart).
...
internal/command/                 Command registry + handlers
...
- **Unit tests** for configuration validation, identity persistence, state collection, backoff scheduling, the HTTP client, and the command registry
- **Runtime tests** against a fake control-plane client: registration + heartbeat, identity reuse across restarts, survival during control-plane downtime, command execution, unknown-identity re-registration, `RESTART_AGENT` shutdown, and the local health endpoint
...
- State reporting: only `status` + `ip_address` are sent to the control plane (Phase 2 contract); detailed machine state stays local
